...
1. **Borrar la key vieja "Authorization" (`sk-...lKIA`)** — ⚠️ **CORRECCIÓN 2026-06-16:** el claim "nada usa la lKIA" era FALSO. El nodo `OpenAI — Generar embedding` de **Onboarding v2** la usaba vía la credencial `Header Auth account` → la key ya estaba muerta (401) y tenía el onboarding roto en silencio. Migrado a `OpenAI W1W2` el 2026-06-16 (verificado end-to-end). Resto: W1, W2 y W4 ya migrados a credenciales (`OpenAI W1W2` + `Hunter W2`). W3 no usa OpenAI. Mateo usa nodo nativo con credencial `openAiApi` aparte. **Ahora sí** nada usa la `sk-...lKIA` → confirmar "Last used" congelado y borrarla en platform.openai.com. NO borrar la key "Timeless" `sk-...b2wA` (activa: Mateo/Emma).
   - **Verificado 2026-06-14 (audit n8n):** la única key OpenAI hardcodeada que queda en los Bot Demos es `sk-...b2wA` (la ACTIVA), en nodos `Code Tool` **deshabilitados** — NO la `lKIA`. La llamada real al LLM usa la credencial `OpenAI account 2`. Confirmado: borrar la `lKIA` no rompe esos nodos.
   - ⚠️ **Riesgo de secrets (nuevo):** la key activa `sk-...b2wA` + el `service_role` de Supabase (`sb_secret_...`) están en texto plano en los Code Tool de los Bot Demos y, completos, en `n8n-workflows/sunlife-bot-demo.json` y `…-fixed.json`. Estos `.json` se agregaron a `.gitignore` (2026-06-14) para evitar commit accidental al repo `timeless-site`. **No hubo filtración pública** (esos `.json` nunca se commitearon/pushearon, verificado). → **NO rotar el `service_role` de apuro:** rotarlo invalida la llave y rompe Emma (Sun Life live) + demos hasta re-configurar. El camino correcto y sin riesgo es **migrar esos secrets hardcodeados a credenciales de n8n** (parte del refactor de los 5 Bot Demos), no una rotación de emergencia.
